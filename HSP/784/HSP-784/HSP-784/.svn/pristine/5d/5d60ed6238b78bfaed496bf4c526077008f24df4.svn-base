// kludge to allow current hotspot feed to work, inserts into SNI.HGTV.Photogallery3 ns
...
(function($) {
	...
	function doDynOmni(i) {
	    ...
	    if (typeof s == "object") {
			omniVars = new Object();
			if (dyn_load_origin != "") {
			    omniVars.prop26 = "HGTV : Photo Gallery : " + config.gallery_title + " : " + dyn_load_origin + " : " + i;
			    dyn_load_origin = "";
			}
	...
	function createImageWrapper(current_data) {
		...
		if (current_data.bhs != -1) {
		    gallery.nextPhoto(e);
		    var omniParams = {
			section: "HGTV Remodels: Photo Gallery",
	...
	function shortenCaptionText(index) {
	    ...
	    truncated_cap = SNI.Util.truncateHtml(cap, caption_length),
	    rendered_content = truncated_cap.success ? truncated_cap.output + '<span class="pg-cap-elipse">&#8230; <span class="pg-show-caption">More</span></span>' : cap;
	...
	function loadPhotoInfo(index, bFirst) {
	    ...
	    // MM-4852: Fire comScore tracking function on navigation to new slide (counts as a new page load)
	    if (typeof(COMSCORE) === "object") {
	        COMSCORE.beacon({
	            c1: "2",
	            c2: "6035648",
	            c3: "",
	            c4: "http://www.hgtvremodels.com" + current_data.pgUrl
	        });
	}
	}

	function verifyWrapperHeight() { //changing height
	    if (top != self) top.SNI.HGRM.adjustFrameWrapperHeight( $('.photo-gallery').height() );
	}
	...
	function applyHotspots(current_data) {
	    if(typeof SNI.HGTV.Photogallery3.Hotspots === "object") {
		$(SNI.HGTV.Photogallery3.Hotspots.images).each(function() {
			...
			var showProducts = function(e) {
 			    SNI.HGRM.Omniture.HotSpotClick(e.data.hotspot, "c");
			...
			hotspots_products.find("ul.pg-hotspot-list li").each(function(index) {
			    $(this).click(function() {
				SNI.HGRM.Omniture.HotSpotClick(hotspotImage.hotspots[index], "c");
				hotspots_products.find("ul.pg-hotspot-list li.selected").removeClass("selected");
				hotspot_items_info.hide();
			...
			hotspots_products.find("ul.pg-hsproducts li").each(function(index) {
				...
				SNI.HGRM.Omniture.HotSpotClick(hotspotImage.hotspots[index], "1");
				e.stopPropagation();
				return true;
			    });
			});
	 ...
	 */
	_load_once = function() {
	    thumbnails_frame.find(".hd span.pg-close-btn").click(function(e) {
	...
	if (config.images[current_index].iid != "endframe") {
	    ...
	    if (typeof config.images[current_index].image_wrapper === 'undefined'){
	...
	if (config.hotspotURL) {
		...
		success: function(data) {
		    ...
		    SNI.HGRM.buildProductsTab = function () {
	...
	gallery_element.find(".pg-error .btn").click(function(e) {
	    gallery.goToPhoto(current_index);
	    e.preventDefault();
	});
	...
	/*
	 * Addon function to SNI.HGRM.DynamicAds to override show of interstitial ads
	 * @addon
	 */
	SNI.HGRM.DynamicAds.init({
    ...
    SNI.HGRM.PhotoGallery = PhotoGallery;
})(jQuery);