...
(function($) {
	...
	o.init = function() {
		...
		$.ajax({url: SNI.Config.kudzuSearchPath + "&callback=SNI.HGRM.KudzuModule.processLoad&searchVal=" + mySearchVal,
            dataType: 'script', 
            cache: true,
            timeout: nTimeout,
            error: SNI.HGRM.KudzuModule.errorLoad });
	};
