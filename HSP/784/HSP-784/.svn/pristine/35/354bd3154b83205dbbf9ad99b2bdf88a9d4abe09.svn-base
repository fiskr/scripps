(function($){
    SNI.HGRM.TopicFilters = function(){
        ...
        getFilteredResults = function (e, keepFilter) {
            ...
            if(keepFilter !== true) {
                $filterList.find("li.sel").removeClass("sel");
        ...
        getFilteredResultsJSON = function () {
            $.ajax({
                url: "/HGRM/cxfservice/resultsByTopic/topic/" + topicParams.topic + "/filter/" + topicParams.filter + "/pageNum/" + topicParams.pageNum + "/count/10/resultsByTopic.js",
                dataType: "json",
                success: drawResults
            });
        },
        ...
        drawResults = function(data) {
            ...
            if(data != null) {
                ...
                SNI.HGRM.DynamicAds.descr.refreshRate = 1;
                // track pageview
                SNI.Nielsen.trackNSE();
                SNI.HGRM.Omniture.pageViewTrack({
                    Url: trackUrl,
        ...
        drawPaginator = function (currentResultPage, totalCount) {
            ...
            if(currentResultPage == 1) {
                paginationHTML += '<span class="nextprev prev">&larr; Previous&nbsp;&nbsp;</span>' + '<span class="current">1</span>';
            } else {
                paginationHTML += '<a href="'+topicParamsUrl+'page-'+parseInt(currentResultPage-1, 10)+'.html" class="nextprev prev">&larr; Previous&nbsp;&nbsp;</a>' +'<a href="'+topicParamsUrl+'page-1.html">1</a>';
            }
            ...
            if(currentResultPage < totalPages) {
                paginationHTML += '<a class="next" href="'+topicParamsUrl+'page-'+parseInt(currentResultPage+1, 10)+'.html">Next &rarr;</a>' + '</div>';
            } else {
                paginationHTML += '<span class="nextprev next">Next &rarr;</span>' + '</div>';
            }
        ...
        drawArticle = function (result) {
            var html = '<div class="pod">';
            html += '<div class="wrap right">';
            html += '<div class="bd">';

            if(result.thumbnail && result.thumbnail != "") {
                html += '<a href="' + result.url + '"><img alt="' + result.title + '" title="' + result.title + '" height="90" width="120" src="' + result.thumbnail + '" /></a>';
            }

            html += '<h4 class="in">' + '<a href="' + result.url + '">' + result.title + '</a> ';

            if(result.contentCount && result.contentCount != "") {
                html += '<cite class="in cap">(' + result.contentCount + ' Photos)</cite>';
            }
        ...
        drawVideo = function (result) {
            ...
            if(result.thumbnail && result.thumbnail != "") {
                html += '<a class="vid-btn" href="' + result.url + '"><img alt="' + result.title + '" title="' + result.title + '" height="90" width="120" src="' + result.thumbnail + '" />'+'<span class="ico"/></a>'
            }
            html += '<h4 class="in">' + '<a href="' + result.url + '">' + result.title + '</a> ';

            if(result.contentCount && result.contentCount != "") {
                html += '<cite class="in cap">(video ' + result.contentCount + ')</cite>'
            }
        ...
        moveAdBox = function () {
            var swapped = false;
            var $sniE = $("#sni-e");
            var $modules = $sniE.children("div");
            var $mrec = $sniE.find(".mrec");
            var $bigBoxAd = $sniE.find(".bigboxad");
        ...
        return {
            initialize: function(page) {
                ...
                SNI.HGRM.DynamicAds.descr.refreshRate = 2;

                $loadingPanel = $('<div class="loader">' +
