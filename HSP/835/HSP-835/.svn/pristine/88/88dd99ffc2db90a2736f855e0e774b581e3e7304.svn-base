...
mergeObjects = function (trg, src) {
    ...
    for (prop in src) {
        ...
        if (typeof (trg[prop]) == 'undefined' || trg[prop].constructor == Array || typeof (trg[prop]) != 'object' || !deep) {
...
if (typeof (SNI.Player.Settings) == "undefined") {
    SNI.Player.Settings = {
        ...
        // --------------------------------------
        snap_swf_url: SNI.Config.snapBinary,
...
// choose Flash SNAP player or HTML5
SNI.Player.wantsHTML5 = function () {
    /// for testing:
    if ( !! SNI.Config.DEBUG && !! SNI.Config.DEBUG.fmt && !! SNI.Config.snapEnableHTML5) return true;

    // production:
    return ( !! SNI.Config.snapEnableHTML5 && /iphone|ipad|ipod/i.test(window.navigator.platform));
};
...
SNI.Player.SNAP = function (config) {
    ...
    if (typeof (mdManager) != "undefined") {
        if (SNI.Player.wantsHTML5()) {
            mdManager.addParameter("VideoPlayer", "SNAPHTML5");
        } else {
...
*/
    ...
    if (mdManager.getParameter("VideoPlayer") === "SNAP") {
        swfobject.embedSWF(
        ...
        attributes);
    } else if (mdManager.getParameter("VideoPlayer") === "SNAPHTML5") {
        this.ui_config.alternativeContent = '<p>The video player is starting up...</p>';
        SNI.Player.SNAPHTML5.init({
            cfgArgs: flashvars,
            dimensions: this.ui_config.dimensions, // for whole player
            enableSyncAdFix: this.ui_config.enableSyncAdFix
        });
        console.log("VideoPlayer = HTML5");
    }
