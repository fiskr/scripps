...
$.ui.accordion = function(container, options) {
	...
	if ( options.fillSpace ) {
		...
		var maxPadding = 0;
		options.headers.direction(options.direction).each(function() {
...
$.ui.accordion.prototype = {
	...
	destroy: function() {
		this.options.headers.direction(this.options.direction).css("display", "");
		if ( this.options.fillSpace || this.options.autoHeight ) {
			this.options.headers.direction(this.options.direction).css("height", "");
		}
...
function clickHandler(event) {
	...
	if ( !event.target && !options.alwaysOpen ) {
		...
		var toHide = options.active.direction(this.options.direction),
			data = {
				instance: this,
				options: options,
				newHeader: jQuery([]),
	...
	var toShow = clicked.direction(options.direction),
		toHide = options.active.direction(options.direction),
