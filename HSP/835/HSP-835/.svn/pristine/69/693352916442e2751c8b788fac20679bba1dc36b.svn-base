...
(function($) {
    ...
    $.extend($.ui.tabs.prototype, {
        ...
        tabify: function(init) {
			...
			// this.$lis = $('h5:has(a[href])', this.source);
            ...
            if (init) {
				...
				// set the width of the navClass and center it with absolute positioning
				if (o.forcePositionCenter && $(this.source).hasClass(o.navClass)) {
					nav = $(this.source);
					
					navWidth = 1; // keep one pixel to fix FF3
					nav.children().each(function() { 
						navWidth += $(this).outerWidth({ margin: true }); 
					});
					
					// sometimes the width calculation seems to be missing a few pixels
					if ($.browser.msie && (parseInt($.browser.version) < 7)) {
						navWidth += 3;
					}

					nav.css({width: navWidth, position: 'absolute', left: '50%'});
					nav.css('margin-left', -1 * Math.ceil(navWidth / 2.0));
				}
