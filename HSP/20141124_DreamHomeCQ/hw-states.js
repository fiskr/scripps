...
(function( $ ) {
	...
	SNI.TP.Sweeps.HelloWorld = {
		...
		"setState" : function( state ) {
			...
			if(state === loading){
				if(!window.location.hostname.match(/author\./g)){
					//if loading state, show the loading cover.
					$('#cover').show();
				}else if(window.location.hostname.match(/wcmmode\=disabled/g)){ //if in preview, put up the cover
					//if loading state, show the loading cover.
					$('#cover').show();
				}else{ //if not preview and still author - hide the cover.
					$("#cover").hide();
				}
			} else{
				$('#cover').hide();
			...
			if ( ( state !== HW.getDefaultState() ) && scroll && ( scroll > 0 ) ) {
				if(state === reg_err || state === loading){
					
					var off = $('.container-site').offset() ? $('.container-site').offset().top : '0px';
		...
		"refreshAds" : function(state, thanks) {
			if(SniAds && typeof SniAds.ready === 'function'){
				SniAds.ready(function(){
					if(googletag && typeof googletag.pubads === 'function' && typeof googletag.pubads().refresh === 'function'){
						if(state === thanks){
							if(typeof googletag.pubads().setTargeting !== 'undefined'){
								// this resets the targeting for a different sponsorship code on thanks (GMC has special ads)...
								googletag.pubads().setTargeting("topic", mdManager.getParameter('Sponsorship')).refresh(); 
							}else{
								console.error('HW-STATES: googletag\'s setTargeting function is undefined.');
						...
						console.error('HW-STATES: googletag object, pubads, or refresh are suspect.');
					}
				});
			}
