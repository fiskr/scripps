...
(function($) {
	...
	handleUserDetails = function(eventName, payload) {
		console.log('EngageSciences: Handling user details, event: "' + eventName + '" and payload: "' + payload.toString() + '".');
	...
	stateUpdate = function(eventName, payload) {
		...
		if ( confirmStates.indexOf(state) > -1 ) {
			...
			confirmationView(true); // make sure confirmationView fires after checking user details
		} else {
	...
	refreshAds = function() {
		...
		if ( window.SniAds && SniAds.refreshSlot ) {
			SniAds.refreshSlot('dfp_bigbox'); //bigbox refresh
			SniAds.refreshSlot('dfp_sponsorship_module'); //300x150
			//SniAds.refreshSlot('dfp_leaderboard'); //leaderboard refresh
			SniAds.refreshSlot('dfp_pushdown_brandscape'); // pushdown brandscape
	...
	confirmationView = function(isThanks) {
		...
		if(isThanks){
			...
			$('h1.stubby-banner').css('background-image', 'url("http://hgtv.sndimg.com/HGTV/2014/09/10/hgrm-dhr2014-banner-994x60-design-v1.jpg")');  
			$('h1.stubby-banner').height('60px'); 
			//init the video
			switch (mdManager.getParameter('Site')){
				case 'DIY':
					playVideo(84561, true);
					break;
				case 'HGTV':
					playVideo(79741, true);
					break;
				case 'HGRM':
					playVideo(79782, true);
					break;
			}
		}
	},
