(function($){
    ...
    var DEFAULT_CONFIG = {
          'gallery_title'            : ''
        , 'current_index'            : 0
        , 'thumbnail_loader'         : 'http://web.hgtv.com/webhgtv/hg20/imgs/hg-loader.gif'
        , 'default_product_image_sm' : 'http://web.hgtv.com/webhgtv/hg20/imgs/product_unavailable_sm.jpg'
        , 'default_product_image_lg' : 'http://web.hgtv.com/webhgtv/hg20/imgs/product_unavailable_al.jpg'
        , 'images'                   : null
        , 'num_images'               : 0
    };
     ...
     * @return PhotoGallery
     */
    SNI.HGTV.PhotoGallery4 = function (element, config) {
	...
	loaded_image_index     = new Array(),
        ...
        prevUrls               = [],
        prevUrlsIndex           = 0; //if YMAL suggestions are exhausted, send users to this index of the history
        ...
        function getGalleryDataUrl(section_id, asset_id) {
            ...
            return '/hgtv/cda/modules/articleMetadata/0,,ARTICLE-DETAIL_HGTV_' + asset_section + '_ARTICLE-DETAIL_no_,00.html';
        }
        ...
        function updateInlineProducts(current_index){            

            if(mdManager.getParameter('DspDelvFrmt') == 'ARTICLE_BUILDER_PG_WAYFAIR' && config && config["images"].length >= current_index+1 && typeof config["images"][current_index].iid !== "undefined") {
                if (!($('#inline-products').length)) $('.pg-viewport .ad-unit').after("<div id='inline-products'> </div>") //failsafe, and for demo
                    var url = ' http://www.wayfair.com/v/inspiration_gallery/hgtv_widget?hgtvPhotoID=' + config["images"][current_index].iid;
                    $('#inline-products').html('<iframe id="wayfair-widget" height="407" width="966" scrolling="no" frameborder="0" src="' + url + '&bpss=yes"></iframe>');
            } else {
                $('#inline-products').html(''); //clear
            }            
        }
        ...
        function getNextGalleryIndex() {
            var i = 0,
            default_index = -1; //when no new galleries are found in ymal_next
                                //case where prevUrls.length == 0 and ymal_next.length == 0 is handled in getGalleryData(), and the value returned here doesn't matter
            ...
            for (i; i < ymal_next.length; i++) {
                ...
                if (prevUrls.length == 0 || prevUrls.indexOf(next_gall_url) == -1 ) return i;

            }
            //want to return the first url in the history to create largest possible loop
            //so return index of that url in current ymal_next
            if (i === ymal_next.length) return default_index;
        ...
        function doDynOmni(i) {
            ...
            if (typeof s === "object") {
                if (dyn_load_origin !== "") {
                    omniVars.prop26 = "HGTV : Photo Gallery : " + config.gallery_title + " : " + dyn_load_origin;
	...
	function preLoadImage(index) {
	    ...
	    if ( loaded_image_index.indexOf(index) != -1 || imageData.length <= index) return;

	    $('div.photo-gallery4').append('<img src="' + imageData[index].iurl + '" style="display:none;" />');
	    loaded_image_index.push(index);
	}
        ...
        function setupToggler(toggler, index) {
            ...
            enlarge.unbind('click').click(function(e){
                e.preventDefault();
                e.stopPropagation();
                viewport.css('height','auto'); //allow viewport to grow
            ...
            shrink.click(function(e){
                e.preventDefault();
                e.stopPropagation();
                viewport.css('height','auto'); //allow viewport to shrink
                current_image.image_wrapper.animate({
                    width: '339px'
                }, 250);
                current_image.image_wrapper.find('a.photo img').animate({
		...
		if (mediastream && config.images.length == 1){//if only one image in gallery
            ...
            if(!on_initial_load){ doDynOmni(index);
                $.ajax({//SE-10512 -- comScore candidate pageview
                    ...
                    error: function () {
                        console.warn('Could not perform comScore candidate pageview');
                    }
                });
            }
            ...
            if(!current_image.image_wrapper) {
                ...
                current_image.image_wrapper.fadeIn('fast');
                 $viewport.css('height','auto'); //similar function in load() in createImageWrapper; putting it there reduces viewport height flicker on next/previous clicks
            };
            ...
            gallery.addDropWallet(current_image);

            // MM-4853: Fire comScore tracking function on navigation to new slide (counts as a new page load)
            // HOME-44: adding the check for undefined beacon method. This will allow for better handling
            ...
            if (typeof(COMSCORE) === "object" && typeof(COMSCORE.beacon) === "function") {
                COMSCORE.beacon({
                    ...
                    c2: "6035648",
                    c3: "",
                    c4: "http://www.hgtv.com" + current_image.pgUrl
                });
            }
        };
        ...
        function applyHotspots(current_data) {
            if (typeof SNI.HGTV.Photogallery3.Hotspots === "undefined") return console.info("There were no hotspot.");

            var _hots = [];

            var imgs = SNI.HGTV.Photogallery3.Hotspots.images,
            len = imgs.length,
            i = 0;

            var Hotspot = function (hotspot) {
                if (!arguments.length && hotspot.id) return console.warn("Cannot construct Hotspot class without parameters.");
            ...
            for (i; i < len; i++) {
                ...
                var showProducts = function(e){
                    ...
                    data = e.data;
                    data.roomName = current_data.ititle;

                    // do omniture
                    SNI.HGTV.Omniture.HotSpotClick( data, 'c' );
                ...
                panel_html.push('</div><a href="javascript:void(0);" class="pg-close-btn pg-close">close</a></div>');
                ...
                hs_products.find('ul.pg-hotspot-list li').click(function(){
                    var $this = $(this),
                    index = $this.index();
                    _hots[index].roomName = current_data.ititle;
                    SNI.HGTV.Omniture.HotSpotClick( _hots[index] , 'c');
                    hs_products.find('ul.pg-hotspot-list li.selected').removeClass('selected');
                    hs_info.hide();
                ...
                hs_products.find('ul.pg-hsproducts li').each(function(index){
                    $(this).find('a.hotspot_readmore_'+index).click(function(e){
                        SNI.HGTV.Omniture.HotSpotClick(_hots[index],'1');
                        e.stopPropagation();
                        return true;
                    });
                });
	    ...
	    addDropWallet: function (current_image) {
	 	if (!current_image.rproduct || isNaN(parseInt(current_image.rproduct)) ) return;
	 	var product_id = current_image.rproduct;

	 	$('.pg-photo-display-wrapper').after('<p class="drop-wallet"></p>');
	 	$('.drop-wallet').append('<span>or</span><a data-dwid="' + product_id + '" href="javascript:void(0);" class="dw_button dw_cart">View Cart</a>');

	 	if (typeof(DW) === 'undefined') {
		    window.affiliate_tag = 'HGTV1011';
	 	    $('.drop-wallet').prepend('<a href="javascript:void(0);" class="dw_product_button" id="' + product_id +  '"></a>');
	 	    var dwsc = document.createElement('script'); dwsc.type = 'text/javascript';
	 	    dwsc.src = 'http://cdnstatic.dropwallet.net/js/dwsdk.min.js';
	 	    var ts = document.getElementsByTagName('script')[0];
	 	    ts.parentNode.insertBefore(dwsc, ts);

	 	} else {
	 	    $('.drop-wallet').prepend('<a href="javascript:void(0);" class="dw_product_button" id="' + product_id +  '"><button id="prod_' + product_id + '">Buy Now</button></a>');

	 	    $('.drop-wallet #prod_' + product_id).click(function(){
	 	        DW.addItem( $(this).parent().attr('id') );
	 	    });

	 	    $('.drop-wallet .dw_cart').click( function(){
	 	        DW.showCart();
	 	    });
	 	}
		...
		function getTrackingVars(module_name, position, id) {
	 	    var base = 'HGTV:DropWallet',
	 	    product_id = (typeof(id) != 'undefined') ? ':' + id : '';

		    s.linkTrackVars='prop14,prop15,prop16,prop17,prop18,eVar16,eVar17,eVar18,eVar19';

	 	    s.prop14 = base;
	 	    s.prop15 = base + ':' + module_name;
	 	    s.prop16 = base + product_id;
	 	    s.prop17 = base + ':N/A';
	 	    s.prop18 = base + ':' + mdManager.getParameter('Url');
	 	    s.eVar16 = base;
	 	    s.eVar17 = base + product_id;
	 	    s.eVar18 = base + ':' + module_name;
	 	    s.eVar19 = base + ':N/A';
		    s.tl(this, 'o', module_name);
		    var x = setTimeout(function() { resetTrackingVars() }, 750);
	 	}

	 	$('a.dw_product_button').click( function() {
	 	    if (typeof s == 'object') getTrackingVars('Buy Now', 1, $(this).attr('id') );
	 	});

	 	$('a.dw_button.dw_cart').click( function() {
	 	    if (typeof s == 'object') getTrackingVars('View Cart', 2, $(this).attr('data-dwid') );
	 	});
	    },

	    removeDropWallet:function() {
	 	$('.drop-wallet button, .drop-wallet div').unbind();
	 	$('.drop-wallet').remove();
	    },

            refreshSocialTools : function(url, title, iurl, ititle){
                ...
                //add Pinterest button -- bypassing core functions to do quickly as this will be revamped by MM-4500
                var pintitle = ititle + " - " + title + " on HGTV";
                $('#toolbar .pinit a').attr('href','http://pinterest.com/pin/create/button/?url='+encodeURIComponent(url) + encodeURIComponent("?soc=sharingpinterest")+"&description="+encodeURIComponent(pintitle)+"&media="+encodeURIComponent(iurl));
                return url;
            },
            ...
            getHubTabs: function(url) {
                $.ajax({
                    ...
                    success: function(data) {
                        $('#toolbar').before(data);
                        setTimeout( function() { SNI.HGTV.highlightHubTab(); }, 1000 );
                    },
                    error: function () {
            ...
            updateGalleryDescArea: function(data) {
                $('div.intro p').html(data.subHeadline); //description
                ...
                SNI.HGTV.Omniture.ClickTrack('li.tags', 'Filed Under: ');
            },

            updateFITG: function(url) { // FITG == Featured In This Gallery
                var container_div  = $('#hg-w .content');
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var t = setTimeout(function() { SNI.HGTV.bindViewMoreEventOnGalleries() }, 1500);
                    },
                    error: function () {
            ...
            getGalleryData: function(redirect_action) {
               ...
               if (redirect_action === 'next') {
                    ...
                    if(ymal_next.length == 0) {console.log("length 0");return 0;}

                    console.log("current_id: " + mdManager.getParameter('DetailId') + " , getNextGalleryIndex= " + i + " , prevUrls=" + prevUrls);

                    if (i === -1) {//all ymal_next options have been visited already
                        var ids = prevUrls[prevUrlsIndex].split('__'); //get id's from prevUrls
                        prevUrlsIndex++;//if exhausted, try next in history next time around
                        if (ids.length !== 2) return;
                        if (ids[0] === mdManager.getParameter('SctnId') && ids[1] === mdManager.getParameter('DetailId')) {
                            //first gallery visited is current gallery
                            console.log("first gallery visited is current gallery")
                        }
                        next_gall_url = getGalleryDataUrl(ids[0],ids[1]);
            ...
            checkHotspotStatus: function() {
                var status = (SNI.HGTV.Photogallery3.Hotspots && SNI.HGTV.Photogallery3.Hotspots.images.length) ? true : false;
            ...
            loadNewGallery : function(data) {
                ...
                document.title = config.gallery_title + ' : ' + catDspName  + ' : HGTV';
                ...
                gallery.updateWeRecommendModule(data.weRecommendModule);
                gallery.refreshSocialTools( config.images[current_index].pgUrl, config.gallery_title, config.images[current_index].iurl,config.images[current_index].ititle);
		...
		adManager.resetKeys();
                ...
                mdManager.setParameter('DspDelvFrmt', data.DspDelvFrmt);
                gallery.updateAds();
            },
            ...
            goToPhoto : function(index){
                ...
                t.closeProductPanel();
                 ...
                 var $viewport = $('.pg-viewport');
                    $viewport.height($viewport.height()); //fix height to reduce viewport flicker
                ...
                if (!isLoading) {
                    if (h !== len && mediastream) {
                        ...
                        next_button.unbind('click').bind('click', gallery.redirect);
                        gallery.redirect_action = 'next';
                    } else {//still refresh ad if not mediastream (or problem with ymal_next)
                        SNI.HGTV.DynamicAds.refresh();
                    }
		    ...
		    if ( SNI.HGTV.hasTouchSupport ) {

			var cacheIndex1 = (index + 1) % config.images.length;
			var cacheIndex2 = (index + 2) % config.images.length;

			preLoadImage(cacheIndex1);
			preLoadImage(cacheIndex2);
		    }
            ...
            redirect: function(e) {
                e.preventDefault();
                var going_to = (gallery.redirect_action == 'next') ? 'Next Gallery' : 'Previous Gallery';

                if (gallery.redirect_action == 'next') next_button.unbind('click', gallery.redirect);

                dyn_load_origin = going_to;
            ...
            previousPhoto: function(e){
                ...
                if ( $(this).data('block') != 'yes' && !previous_button.hasClass('pg-disabled') ) {
                    ...
                    if( gallery_element.hasClass('pg-errored') ) gallery_element.removeClass('pg-errored');
		...
		var myShowNumber = 30;
            ...
            showThumbnails : function(e){
                var $viewport=$('.pg-viewport');
                 $viewport.height($viewport.height()); //set height to current value when thumb button clicked
                 var container = $(".pg-thumbnails-frame");
                    container.height($viewport.height()-70); //make thumbnail button extend to bottom w/ a small gap
        ...
        gallery.initGallery  = function(new_gallery) {
            ...
            if (!new_gallery) {
		if ( SNI.HGTV.hasTouchSupport ) {
		    preLoadImage(1 % imageData.length);
		    preLoadImage(2 % imageData.length);
		}
            ...
            //populate pinterest button on initial load -- doing this quick as MM-4500 will redo this
            $(function(){
                //console.log("pint");                
                var pintitle = (typeof imageData != 'undefined' && imageData[0]) ? imageData[0].ititle + " - " : "";
                pintitle += mdManager.getParameterString('title') + " on HGTV";
                var pinurl = 'http://' + SNI.Config.domain + '/' + mdManager.getParameterString("url"),
                pinmedia = (typeof imageData != 'undefined' && imageData[0]) ? imageData[0].iurl : "http://hgtv.sndimg.com/HGTV/2012/07/02/HGTV_favicon_s92x69.png";
            ...
            updateInlineProducts(current_index); //the wayfair module
                    ...
                    success  : function(data) {
                        ...
                        SNI.HGTV.buildProductsTab = function () {
                            ...
                            $(config.images).each(function(){
                                ...
                                $(SNI.HGTV.Photogallery3.Hotspots.images).each(function() {
            ...
            if ( !new_gallery ) {
                ...
                $(".photo-gallery4 .pg-close-btn").unbind("click", gallery.showThumbnails).bind("click", gallery.showThumbnails);
                ...
                SNI.HGTV.Omniture.ClickTrackSingle('div.recommend', 'You Might Also Like: '.replace('&rsquo;','\u0027'), mdManager.getParameter('title'));
                SNI.HGTV.Omniture.ClickTrackSingle('div.pg-you-might-like', 'You Might Also Like: '.replace('&rsquo;','\u0027'), mdManager.getParameter('title'));

                /*
                 * Addon function to SNI.HGTV.DynamicAds to override show of interstitial ads
                 * @addon
                 */
		SNI.HGTV.PhotoGallery4.callSwipeForElement = function() {
		    SNI.HGTV.PhotoGallery4.swipeForElement();
		}
                ...
                SNI.HGTV.DynamicAds.init({
                    ...
                    'callback': SNI.HGTV.PhotoGallery4.callSwipeForElement,
                    'dismiss_elts': [next_button, previous_button],
                    'video_hide_elts' : [next_button, previous_button]
                });
            ...
            var buildProductsPanel = function( callback, finishedCallback ){
                ...
                var success = function(){
                    var cnt  = 0
                      , imgs = SNI.HGTV.Photogallery3.Hotspots.images
                      , len  = imgs.length
                      , panelsIndex = len
                    ...
                    if (len === 0) {
                        ...
                        return console.warn("There was no products returned from SNI.HGTV.Photogallery3.Hotspots");
                    };
                ...
                if (SNI.HGTV.Photogallery3.Hotspots === "undefined") {
                    $.ajax({
                        "url"      : SNI.HGTV.Photogallery3.hsJSON
                      , "dataType" : "script"
                      , "success"  : success
                    });
                } else {
            ...
            var productDetailsHandler = function(){
                ...
                items.bind("click", function(event){
                    ...
                    var self           = $(this)
                      ...
                      , index          = liList.index(self.parents("li"))
                    ...
                    var populate_wrapper_with_product = function(image, data, isFirstOrLast){
                        ...
                        var product_pagination_next = $("<a class=\"detail_next\" />")
                              ...
                              .data("method", "next")
                              .html("Next Product <span class=\"icon\"></span>");
                        ...
                        var l = SNI.HGTV.Photogallery3.Hotspots.images.length,
                      	imageArray = SNI.HGTV.Photogallery3.Hotspots.images;
                        for(i = 0; i < l; i++) {
                        ...
                        product_detail_more.click(function(e){
                        	var oHS = SNI.HGTV.Photogallery3.Hotspots.images[image.hsIndex].hotspots[0];
                        	SNI.HGTV.Omniture.HotSpotClick(oHS, "l");
                          e.stopPropagation();
                    		});
	...
	SNI.HGTV.PhotoGallery4.swipeForElement = function(el) {
		...
		if(ev.direction == 'right') {
		    $('.pg-previous').trigger('click');
		} else if(ev.direction == 'left') {
		    $('.pg-next').trigger('click');
		}
	    }
	...
	SNI.HGTV.PhotoGallery4.swipe = function() {
	    /* Set-up Hammer */
	    var photo_wrap = $('.pg-photo-display-wrapper');
	    SNI.HGTV.PhotoGallery4.swipeForElement(photo_wrap);
	}

	//load hammer.js if device has touch support
	if ( SNI.HGTV.hasTouchSupport ) {
	    SNI.HGTV.loadMobileJS(SNI.HGTV.PhotoGallery4.swipe);
	}
    }
