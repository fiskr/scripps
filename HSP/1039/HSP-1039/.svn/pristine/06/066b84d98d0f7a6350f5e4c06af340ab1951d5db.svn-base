...
jQuery.dpl = {
	...
	dynlead: function(ele, config){
		...
		pageLinks.find("ul > li:first-child").addClass("active"); // select first in paging
		pageLinks.find("ul > li").each(function(i){ // for all page links apply hover event and click event	
			var $this = $(this);
		...
		var nextSlide = function(ele, config){
			...
			pageLinks.find("ul > li:eq(" + currentSlide + ")").addClass("active");			
