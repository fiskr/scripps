...
namespace eval ::com.scripps::cr::cda::landing::db {
	...
	proc getPhotoGalleriesInTentpole {Site TentpoleId {OrderBy "ALPHA"}} {
		...
		set hubIdList "('$TentpoleId')"

		if {$TentpoleId == "36081"} {
			set hubIdList "('36081', '23461', '10824', '3041' )"
		}

	
		set sqlString ""
		set sqlString "select t2.headline, t2.last_updt_dt, t2.create_dt, t2.image_id, t2.ctnt_type, t2.table_name, t2.cmpn_id, t2.site, t2.delv_status, t2.wflw_status, t2.url, t2.hub_id, count 
from (
select a.headline, a.last_updt_dt, a.create_dt, ac.cmpn_id as image_id, tp.cmpn_ctnt_type as ctnt_type, tp.cmpn_table_name as table_name, tp.cmpn_id, tp.site, a.delv_status, a.wflw_status as wflw_status, u.url as url, tp.hub_id
from (
    ...
    "				
		switch -- $OrderBy {
			"ALPHA" {
			 ...
			 "LAST-UPDATE" {
			 	append sqlString "order by hub_id, last_updt_dt desc"	
			 }
			 "NEW-TO-OLD" {
